...
namespace Geometry
{
	...
	inline MeshData<VertexType, IndexType> CreateCylinder(float radius, float height, UINT slices, const DirectX::XMFLOAT4 & color)
	{
		...
		// 放入底端圆心
		vertexData = { XMFLOAT3(-h2, 0.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f),
		...
		{
			theta = i * per_theta;
			vertexData = { XMFLOAT3(-h2, radius * cosf(theta), radius * sinf(theta)), XMFLOAT3(0.0f, 1.0f, 0.0f),
	...
	inline MeshData<VertexType, IndexType> CreateCylinderNoCap(float radius, float height, UINT slices, const DirectX::XMFLOAT4 & color)
	{
		...
		{
			theta = i * per_theta;
			vertexData = { XMFLOAT3(-h2, radius * cosf(theta), radius * sinf(theta)), XMFLOAT3(cosf(theta), 0.0f, sinf(theta)),
