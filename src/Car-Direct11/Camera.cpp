...
DirectX::XMFLOAT4X4 Camera::GetTargetPositionM() const
{
	return m_TargetM;
}

void Camera::SetTargetM(const DirectX::XMFLOAT4X4 & target)
{
	m_TargetM = target;
}

void Camera::SetMoveState(float state)
{
	moveFlag = state;
}

float Camera::GetMoveState()
{
	return moveFlag;
}
...
void FirstPersonCamera::Turn(int direction)
{
	XMMATRIX Rotation = XMMatrixRotationY(m_rotation*direction);
	XMMATRIX Translation = XMLoadFloat4x4(&m_TargetM);
	XMFLOAT4X4 Ret;
	XMStoreFloat4x4(&Ret, XMMatrixMultiply(Rotation, Translation));
	m_TargetM = Ret;
	return;
}
...
void FirstPersonCamera::Walk(float d)
{
	...
	XMVECTOR Pos = XMLoadFloat3(&m_Position);
	//XMVECTOR Right = XMLoadFloat3(&m_Right);
	XMFLOAT4X4 adjustedPosM = m_TargetM;
	XMVECTOR Right = XMVectorSet(adjustedPosM(0, 0), adjustedPosM(0, 1), adjustedPosM(0, 2), 0.0f);
...
{
	XMVECTOR Pos = XMLoadFloat3(&m_Target);
	XMVECTOR Right = XMLoadFloat3(&m_Right);
	XMVECTOR Dist = XMVectorReplicate(d);
	// DestPos = Dist * Right + SrcPos
	XMStoreFloat3(&m_Target, XMVectorMultiplyAdd(Dist, Right, Pos));
}

void ThirdPersonCamera::Turn(int direction)
{
	XMMATRIX Rotation = XMMatrixRotationY(m_rotation*direction);
	XMMATRIX Translation = XMLoadFloat4x4(&m_TargetM);
	XMFLOAT4X4 Ret;
...
void ThirdPersonCamera::Walk(float d)
{
	...
	XMVECTOR Dist = XMVectorReplicate(d);
	// DestPos = Dist * Front + SrcPos
	XMStoreFloat3(&m_Target, XMVectorMultiplyAdd(Dist, Front, Pos));
}
